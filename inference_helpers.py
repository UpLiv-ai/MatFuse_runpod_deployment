...
def process_palette(palette_source, image_resolution, device):
    # FIX: Handle None case manually to avoid crash
    if palette_source is None:
        # Return 5 black colors (Range 0-1)
        return torch.zeros(5, 3, device=device)

    # FIX: Use PIL Quantization instead of the broken Pylette/KMeans
    # This achieves the same goal (5 dominant colors) but is stable.
    try:
        # Quantize to 5 colors
        quantized = palette_source.quantize(colors=5, method=2) 
        # Get palette (returns flat list [r,g,b, r,g,b...])
        palette_data = quantized.getpalette()[:15] 
        
        # Pad with the last color if we found fewer than 5
        while len(palette_data) < 15:
            palette_data += palette_data[-3:]
            
        # Reshape to (5, 3) and Normalize 0-255 -> 0.0-1.0
        palette_arr = np.array(palette_data).reshape(5, 3)
        return torch.tensor(palette_arr, device=device).float() / 255.0
